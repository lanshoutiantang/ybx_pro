...
def main():
    ...
    maps, intrinsics = infer_old_sensing.get_rectification_parameters(
        args.stereo_param_path
    )
    ...
    for output_dir_path in overall_progress:
        ...
        for depth_path in inner_progress:
            ...
            if args.clear_edges:
                # 检查是否为单通道（2D数组）并且高度至少为32
                if len(depth_image.shape) == 2 and depth_image.shape[0] >= 32:
                    depth_image[-32:, :] = 0  # 将最后32行置为0
                else:
                    tqdm.write(
                        f"  └─ 警告: 深度图 '{base_filename}' 不是单通道或高度不足32，无法执行底部清零。"
                    )
