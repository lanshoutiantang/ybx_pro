...
def plot_multi_dirs(log_dirs, labels, save_path):
    ...
    # colors = ["r", "g", "b", "m", "c"]  # 可扩展
    colors = [
        "tab:red", "tab:green", "tab:blue", "tab:orange", "tab:purple",
...
if __name__ == "__main__":
    ...
    # 待遍历的子目录列表
    level2_dirs = [ "carry_head",  "carry_waist","carry_shiyan","carry"]
    # level2_dirs = [ "carry_shiyan","carry"]

    # 版本映射：key=版本目录名，value=曲线显示的标签（可自定义）
    versions = [
        # "s012_1920_bright",
        # "s015_1920_bright",
        # "s018_1920_new_bright",
        # "s019_1920_new_bright",
        # 如需扩展版本，直接添加即可（示例："s012_1920_ceiling": "s0.1.2_1920-ceiling"）
        # dark系列
        # "s012_1920_dark",
        # "s015_1920_dark",
        # "s018_1920_new_dark",
    ...
    for item in level2_dirs:
        ...
        # 处理「目录组」（列表类型）
            ...
            group_name = "carry_group"  # 给分组命名（可自定义）
            # 遍历分组内的每个子目录 + 所有版本
            for sub_dir in item:
                for ver_dir in versions:
                    full_log_path = os.path.join(base_log_path, ver_dir, sub_dir)
