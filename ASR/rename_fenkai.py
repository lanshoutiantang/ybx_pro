...
if __name__ == '__main__':
	...
	outer_path = r'D:\桌面\UBT\2024\WalkerS_FaceRec\3_11chongxinyanzheng\3_4_zhenglian'
	...
	for folder in folderlist:
		...
		for filename in filelist:
				...
				print(int(0))
				# 0~3米 和 正脸
				# if float(dier) >= float(0) and float(dier) <= float(3):
				# 	if int(disan) == int(0):
				# 		fenkai_infos(inner_path, filename)
				# 		continue
				# 	else:
				...
				# 	os.remove(os.path.join(inner_path, filename))
				# 3~4米 和 正脸
				if float(dier) >= float(3) and float(dier) <= float(4):
					if int(disan) == int(0):
						fenkai_infos(inner_path, filename)
					else:
						os.remove(os.path.join(inner_path, filename))
